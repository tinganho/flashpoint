...
namespace flashpoint::lib {
    ...
    MemoryPool::MemoryPool(const std::size_t total_size, const std::size_t block_size)
    {
    ...
    MemoryPoolTicket* MemoryPool::take_ticket() 
    {
        ...
        MemoryPoolTicket* ticket = (MemoryPoolTicket*)(start_address_of_pool + start_address_of_block(block));
        ticket->block = block;
        ticket->offset = 0;
        return ticket;
    }
    ...
    void* MemoryPool::allocate(const std::size_t size, const std::size_t alignment, MemoryPoolTicket* ticket)
    {
        std::size_t padding = 0;
        const std::size_t current_address = (std::size_t)(start_address_of_pool + start_address_of_block(ticket->block) + ticket->offset);
